 ...
 * {@code mysearch.cache.error} counter so a Redis outage degrades performance but
...
public class SearchCacheService {
    ...
    public Optional<SearchResponse> get(String key) {
        ...
        try {
            ...
            if (raw == null) {
                meters.counter("mysearch.cache.miss").increment();
                return Optional.empty();
            }
            SearchResponse resp = mapper.readValue(raw, SearchResponse.class);
            meters.counter("mysearch.cache.hit").increment();
            return Optional.ofNullable(resp);
        } catch (Exception ex) {
            meters.counter("mysearch.cache.error", "op", "get").increment();
    ...
    public void putAsync(String key, SearchResponse resp) {
        ...
        CompletableFuture.runAsync(() -> put(key, resp), providerExecutor)
                .exceptionally(ex -> {
                    meters.counter("mysearch.cache.error", "op", "put_async").increment();
    ...
    void put(String key, SearchResponse resp) {
        try {
            ...
            meters.counter("mysearch.cache.error", "op", "serialize").increment();
            log.warn("Cache serialize failed for key {}: {}", key, ex.toString());
        } catch (Exception ex) {
            meters.counter("mysearch.cache.error", "op", "put").increment();
    ...
    long computeTtlSeconds(SearchResponse resp) {
        MysearchProperties.Cache c = props.getCache();
